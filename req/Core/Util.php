...
class Util
{
	...
	static function getBrowserType(): string
	{
		...
		foreach (
			...
			if (mb_strstr($ua, $k) !== false)
				if ($k == self::BROWSER_TYPE_ANDROID && mb_strstr($ua, self::BROWSER_TYPE_MOBILE) !== false)
					return $browserType = self::BROWSER_TYPE_ANDROIDMOBILE;
				else if ($k == self::BROWSER_TYPE_IPHONE && mb_strstr($ua, "Safari") === false)
	...
	static function getMobileType(): string
	{
		...
		if ($host === false)
			return self::MOBILE_TYPE_UNKNOWN;
	...
	{
		if (self::canGetMobileUniqueID() && ($id = self::getMobileUniqueID()) !== null)
			return $id;
	...
	{
		return array_reduce(array_map(fn(string $x): int => (int)filemtime($x), get_included_files()), fn(int $x, int $y): int => max($x, $y), 0);
	}
	...
	static function getAbsoluteUrl(string $path = ""): string
	{
		...
		if (
			$linkType == Configuration::LINK_REWRITE ||
			$linkType == Configuration::LINK_AUTO &&
			is_file(".htaccess") &&
			($content = file_get_contents(".htaccess")) !== false &&
			mb_strstr($content, Util::INDEX_FILE_NAME) !== false)
	...
	static function getPathInfo(): string
	{
		...
		if ($pathInfo)
			...
			return $pathInfo = mb_substr((string)mb_strstr(self::getPhpSelf(), self::INDEX_FILE_NAME), mb_strlen(self::INDEX_FILE_NAME));
	}

	/** @return string[] */
	static function splitTags(string $tags): array
	{
		if (self::isEmpty($tags))
			return [];

		$split = preg_split("/ +/", mb_convert_kana(trim($tags), "s"), -1, PREG_SPLIT_NO_EMPTY);
		if ($split === false) return [$tags];
	...
	{
		$input = (string)mb_convert_encoding($input, "UTF-8", "UTF-8,Windows-31J,eucJP-win");
		...
		$input = (string)mb_convert_encoding($input, "UTF-8");
	...
	static function wildcard(string $pattern, string $string): bool
	{
		return (bool)preg_match("/^" . strtr(preg_quote($pattern, "/"), array(
			'\*'	=> '.*',
			'\?'	=> '.',
			'\[\!'	=> '[^',
			'\['	=> '[',
	...
	{
		return (string)preg_replace('@[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]@', " ", $input);
	}

	private static function decodeNumericEntity(string $str, ?string $encoding = null): string
	{
		$result = (string)preg_replace_callback("/&#x([\\dA-F]+);?/i", fn(array $matches) => "&#" . intval($matches[1], 16) . ";", $str);
		$result = (string)preg_replace("/(&#\\d+);?/", "\\1;", $result);
		$convmap = array(
			0x000020,
			0x000020,
			0,
	...
	private static function escapeAmpersand(string $str, bool $is_xhtml = true): string
	{
		...
		$result = (string)preg_replace("/&amp;#(\\d+|x[\\da-f]+)(;?)/i", "&#\\1\\2", $result);
		$result = (string)preg_replace("/&amp;(" . implode("|", $entities) . ")\\b/", "&\\1", $result);
	...
	static function readLines(string $file, bool $skipEmptyLines = false): array
	{
		...
		while (!feof($fp) && ($fgets = fgets($fp)) !== false) {
			$l = self::trimLineBreak($fgets);
	...
	static function convertLineToThreadEntry(string|array $line, ?ThreadEntry $entry = null): ?ThreadEntry
	{
		...
		if (!$entry)
			$entry = new ThreadEntry(intval(strtr($id, array(".dat" => ""))));

		$d = date_parse($lastUpdate);
		$entry->lastUpdate = (int)mktime($d["hour"], $d["minute"], $d["second"], $d["month"], $d["day"], $d["year"]);
	...
	private static function convertAndSaveToThreadInternal(PDO $db, PDO $idb, ?int $subject, string|array $dat, string $com, string $aft, bool $whenContainsWin31JOnly, bool $allowSaveCommentsOnly, bool &$save, ?ThreadEntry $entry): ?Thread
	{
		...
		$data = is_array($dat) ? $dat : [];

		if (!is_array($dat))
			foreach (self::readLines($dat) as $i) {
				$data[] = $j = (string)mb_convert_encoding($i, "UTF-8", "Windows-31J");

				if ($whenContainsWin31JOnly && !$containsWin31JOnly && $j != (string)mb_convert_encoding($i, "UTF-8", "SJIS"))
					$containsWin31JOnly = true;
			}

		$firstline = array_shift($data);
		if ($firstline === null) throw new ApplicationException(".dat の最初の行が読み込めませんでした");

		$line = array_map(fn($_) => html_entity_decode($_, ENT_QUOTES), explode("<>", $firstline)) + array_fill(0, is_array($dat) ? 16 : 15, null);

		if (!is_array($dat))
			array_unshift($line, basename($dat));

		if ($entry === null)
			$entry = self::convertLineToThreadEntry($line) ?? ThreadEntry::create($db);
		...
		$thread->entry->size = round((float)strlen(bin2hex((string)mb_convert_encoding($thread->body, "SJIS-Win", "UTF-8"))) / 2.0 / 1024.0, 2);
	...
	static function convertLinesToCommentsAndEvaluations(int $entryID, array $lines): array
	{
		...
		foreach ($lines as $i) {
			...
			$d = date_parse($dateTimeString);
			$id = (int)mktime($d["hour"], $d["minute"], $d["second"], $d["month"], $d["day"], $d["year"]);

			$comment = Comment::forDateTime($id);
			$comment->body = $body;
			$comment->name = $name;
			$comment->mail = $mail;
			$comment->hash = $hash;
			...
			if ($comment->body != "#EMPTY#") {
				$comment->body = strtr($body, array("<br />" => "\r\n"));
				$rt[] = $comment;
			}
