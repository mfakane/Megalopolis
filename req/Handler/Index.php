...
class IndexHandler extends Handler
{
	...
	function index(string $_subject = "0", string $_id = "0"): bool
	{
		...
		$entries = $dh->executeCombo($idh, function ($db, $idb) use ($subject) {
			if ($subject == 0)
		...
		switch (App::$handlerType) {
			case "json":
				return Visualizer::json(array(
					"entries" => array_values(array_map(fn($x) => $x->toArray(Configuration::ON_SUBJECT), $entries)),
					"subject" => $this->subject,
					"subjectCount" => $this->subjectCount
				));
			case "csv":
				return self::toCSV($entries);
			case "atom":
	...
	function recent(): bool
	{
		...
		if ($view !== null)
			foreach (explode(",", $view, Configuration::$instance->maxHistory) as $i)
				if ($entry = $dh->execute(fn($db) => ThreadEntry::load($db, intval($i))))
	...
	function search(): bool
	{
		...
		$dh = App::openDB();
		$idh = App::openDB(App::INDEX_DATABASE);
		$vals = $dh->execute(fn($db) => ThreadEntry::getMaxMinValues($db));
		$query = array(
		...
		if (
			...
			if (self::param("random") == "true") {
				$rt = $dh->executeCombo($idh, fn($db, $idb) => ThreadEntry::search($db, $idb, $query, 0, 0, ThreadEntry::SEARCH_RANDOM));
			...
			$rt = $dh->executeCombo(
				...
				ThreadEntry::search(
					...
					$idb,
					$query,
					($this->page - 1) * Configuration::$instance->searchPaging,
	...
	function random(): bool
	{
		$dh = App::openDB();
		$idh = App::openDB(App::INDEX_DATABASE);
		$entry = $dh->executeCombo($idh, fn($db, $idb) => ThreadEntry::getRandomEntry($db, $idb));
		$idh->close();
		$dh->close();
	...
	function author(): bool
	{
		...
		if ($isNameList || !isset($name)) {
			$pageCount = (int)ceil(($this->entryCount = $dh->execute(fn($db) => ThreadEntry::getNameCount($db))) / Configuration::$instance->tagListing);
			Visualizer::$data = ThreadEntry::getNames(
				$dh->execute(fn($db) => $db),
				$page * Configuration::$instance->tagListing,
				Configuration::$instance->tagListing
			);
		} else {
			if ($name == "random") {
				$arr = $dh->execute(fn($db) => ThreadEntry::getNames($db));
				$dh->close();
			...
			if (strstr($name, "/random") == "/random") {
				...
				$this->entries = $dh->execute(fn($db) => ThreadEntry::getEntriesByName($db, $name));
			...
			$this->entries = $dh->execute(fn($db) => ThreadEntry::getEntriesByName($db, $name, $page * Configuration::$instance->searchPaging, Configuration::$instance->searchPaging, Board::ORDER_DESCEND, $this->entryCount));
			...
			$pageCount = $dh->execute(fn($db) => (int)ceil((is_null($this->entryCount) ? ThreadEntry::getEntryCountByName($db, $name) : $this->entryCount) / Configuration::$instance->searchPaging));
			$this->subject = 0;
			$this->subjectCount = $dh->execute(fn($db) => Board::getLatestSubject($db));
	...
	function tag(): bool
	{
		...
		if ($isTagList || !isset($tag)) {
			$pageCount = ceil(($this->entryCount = $dh->execute(fn($db) => ThreadEntry::getTagCount($db))) / Configuration::$instance->tagListing);
			Visualizer::$data = $dh->execute(fn($db) => ThreadEntry::getTags($db, $page * Configuration::$instance->tagListing, Configuration::$instance->tagListing));
		} else {
			if ($tag == "random") {
				$arr = $dh->execute(fn($db) => ThreadEntry::getTags($db));
				$dh->close();
			...
			if (strstr($tag, "/random") == "/random") {
				...
				$this->entries = $dh->execute(fn($db) => ThreadEntry::getEntriesByTag($db, $tag));
			...
			$this->entryCount = null;
			$this->entries = $dh->execute(fn($db) => ThreadEntry::getEntriesByTag($db, $tag, $page * Configuration::$instance->searchPaging, Configuration::$instance->searchPaging, Board::ORDER_DESCEND, $this->entryCount));
			...
			$pageCount = $dh->execute(fn($db) => ceil((is_null($this->entryCount) ? ThreadEntry::getEntryCountByTag($db, $tag) : $this->entryCount) / Configuration::$instance->searchPaging));
			$this->subject = 0;
			$this->subjectCount = $dh->execute(fn($db) => Board::getLatestSubject($db));
			Visualizer::$data = $tag;
		}
