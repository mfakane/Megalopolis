...
class Template
{
	...
	static function entryInfo(int &$idx, ThreadEntry $i, array $visibility, string $label, string $member, mixed $value): void
	{
		...
		<dd class="<?php echo $member . ($visible ? "" : " hidden") ?>">
			<?php Visualizer::converted((string)$value) ?>
		</dd>
		<?php
	}
	 ...
	 * @param array<string, int> $visibility
	 */
	static function entryInfoHeaderSingle(array $visibility, string $member, mixed $header): void
	{
		?>
		<th class="info <?php echo $member ?><?php echo isset($visibility[$member]) ? null : " hidden" ?>">
			<?php Visualizer::converted((string)$header) ?>
		</th>
		<?php
	}
	 ...
	 */
	static function filterVisibleOnly(array $visibility, array $arr): array
	{
		$rt = array();
	 ...
	 * @param "compact"|"double"|null $listType
	 */
	static function entries(array $entries, bool $isAdmin, ?string $listType = null): void
	{
					...
					<li>
						...
						<?php if ($isAdmin
							...
							<dl>
								<?php
								$idx = 0;
								$v = array_flip(explode(",", "evaluationCount,commentCount,points,rate"));
								?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $v, "評価", "evaluationCount", $c->pointMap && $c->commentPointMap ? "{$i->commentedEvaluationCount}/{$i->evaluationCount}" : $i->evaluationCount) ?>
								<?php if ($isAdmin || $c->showComment[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $v, "コメント", "commentCount", $i->commentCount) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $v, "POINT", "points", $i->points) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showRate[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $v, "Rate", "rate", sprintf("%.2f", $i->rate)) ?>
							</dl>
						<?php endif ?>
					</li>
				<?php endforeach ?>
					...
					<article>
						<div class="articleBody">
							<?php if ($isAdmin || $c->showTitle[Configuration::ON_SUBJECT]): ?>
								<h2>
									<?php if ($isAdmin): ?>
										<input type="checkbox" name="id[]" value="<?php Visualizer::converted(strval($i->id)) ?>" />
									<?php endif ?>
									<a href="<?php Visualizer::converted(Visualizer::actionHrefArray(array($i->subject, $i->id))) ?>"><?php Visualizer::converted($i->title) ?></a>
								</h2>
							<?php endif ?>
							...
							<dl>
								<?php $idx = 0; ?>
								<?php if ($isAdmin || $c->showPages[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "ページ", "pageCount", $i->pageCount) ?>
								<?php if ($isAdmin || $c->showSize[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "サイズ", "size", "{$i->size}KB") ?>
								<?php if ($isAdmin || $c->showReadCount[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "閲覧", "readCount", $i->readCount) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "評価", "evaluationCount", $c->pointMap && $c->commentPointMap ? "{$i->commentedEvaluationCount}/{$i->evaluationCount}" : $i->evaluationCount) ?>
								<?php if ($isAdmin || $c->showComment[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "コメント", "commentCount", $i->commentCount) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "POINT", "points", $i->points) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showRate[Configuration::ON_SUBJECT]) self::entryInfo($idx, $i, $visibility, "Rate", "rate", sprintf("%.2f", $i->rate)) ?>
							</dl>
							<?php endif ?>
		...
		else
		{
			...
			(
				...
				"title" => $isAdmin || $c->showTitle[Configuration::ON_SUBJECT],
				"name" => $isAdmin || $c->showName[Configuration::ON_SUBJECT],
			))) + count(self::filterVisibleOnly($visibility, array
			(
				"dateTime" => true,
				"lastUpdate" => true,
			))) + count(self::filterVisibleOnly($visibility, array_filter(array
			(
				"pageCount" => $isAdmin || $c->showPages[Configuration::ON_SUBJECT],
				"size" => $isAdmin || $c->showSize[Configuration::ON_SUBJECT],
				"readCount" => $isAdmin || $c->showReadCount[Configuration::ON_SUBJECT],
				...
				<table>
					<thead>
						<tr>
							...
							<?php if ($isAdmin || $c->showPages[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "pageCount", "ページ") ?>
							<?php if ($isAdmin || $c->showSize[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "size", "サイズ") ?>
							<?php if ($isAdmin || $c->showReadCount[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "readCount", "閲覧") ?>
							<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "evaluationCount", "評価") ?>
							<?php if ($isAdmin || $c->showComment[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "commentCount", "コメント") ?>
							<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "points", "POINT") ?>
							<?php if ($isAdmin && $c->useAnyPoints() || $c->showRate[Configuration::ON_SUBJECT])  self::entryInfoHeaderSingle($visibility, "rate", "Rate") ?>
						</tr>
					</thead>
					<tbody>
						<?php foreach ($entries as $idx => $i): ?>
							<tr class="article<?php if (!$isAdmin && !$c->showTags[Configuration::ON_SUBJECT] && !$c->showSummary[Configuration::ON_SUBJECT]) { echo ' notags'; } else if ($c->useSummary && !Util::isEmpty($i->summary)) { echo ' hasSummary'; } ?>" id="article<?php echo $i->id ?>">
								...
								<?php endif ?>
								<td class="dateTime<?php echo isset($visibility["dateTime"]) ? null : " hidden" ?>" data-unixtime="<?php Visualizer::converted(strval($i->dateTime)) ?>">
								...
								<?php if ($isAdmin || $c->showReadCount[Configuration::ON_SUBJECT]) self::entryInfoSingle($i, $visibility, "readCount", $i->readCount) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT])  self::entryInfoSingle($i, $visibility, "evaluationCount", $c->pointMap && $c->commentPointMap ? "{$i->commentedEvaluationCount}/{$i->evaluationCount}" : $i->evaluationCount) ?>
								<?php if ($isAdmin || $c->showComment[Configuration::ON_SUBJECT])  self::entryInfoSingle($i, $visibility, "commentCount", $i->commentCount) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showPoint[Configuration::ON_SUBJECT])  self::entryInfoSingle($i, $visibility, "points", $i->points) ?>
								<?php if ($isAdmin && $c->useAnyPoints() || $c->showRate[Configuration::ON_SUBJECT])  self::entryInfoSingle($i, $visibility, "rate", sprintf("%.2f", $i->rate)) ?>
							</tr>
									...
									<td colspan="<?php echo $spanWidth ?>">
										...
										<?php if ($c->useSummary && ($c->showSummary[Configuration::ON_SUBJECT] || $isAdmin) && !Util::isEmpty($i->summary)): ?>
											<p class="summary<?php echo isset($visibility["summary"]) ? null : " hidden" ?>">
												<?php Visualizer::convertedSummary(strval($i->summary)) ?>
											</p>
										<?php endif ?>
									</td>
								</tr>
